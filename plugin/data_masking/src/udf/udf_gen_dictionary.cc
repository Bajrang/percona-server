...
static bool gen_dictionary_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  DBUG_ENTER("gen_dictionary_init");

  if (args->arg_count != 1)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument list: gen_dictionary(dictionary name)");
    DBUG_RETURN(true);
  }

  if (args->arg_type[0] != STRING_RESULT)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument type: gen_dictionary(string)");
    DBUG_RETURN(true);
  }
...
*
* @param dictionary_name: A string that names the dictionary from which to choose the term.
*
* @return A random term from the dictionary as a string, or NULL if the dictionary name is not in the dictionary registry.
*/
...
static char * gen_dictionary(UDF_INIT *, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *)
{
...
udf_descriptor udf_gen_dictionary()
{
  return {"gen_dictionary",
          ...
          reinterpret_cast<Udf_func_any>(gen_dictionary),
          gen_dictionary_init,
