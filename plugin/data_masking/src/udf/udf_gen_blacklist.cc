...
static bool gen_blacklist_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  DBUG_ENTER("gen_blacklist_init");

  if (args->arg_count != 3)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument list: gen_blacklist(str, dictionary_name, replacement_dictionary_name)");
    DBUG_RETURN(true);
  }

  if (args->arg_type[0] != STRING_RESULT || args->arg_type[1] != STRING_RESULT || args->arg_type[2] != STRING_RESULT)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument type: gen_blacklist(string, string, string)");
    DBUG_RETURN(true);
  }
...
* @param dictionary_name: A string that names the dictionary containing the term to replace.
* @param replacement_dictionary_name: A string that names the dictionary from which to choose the replacement term.
*
* @return A string randomly chosen from replacement_dictionary_name as a replacement for str, or str if it does not appear in dictionary_name, or NULL if either dictionary name is not in the dictionary registry.
*          If the term to replace appears in both dictionaries, it is possible for the return value to be the same term.
*/
static std::string _gen_blacklist(const char *str, const char *dictionary_name, const char *replacement_dictionary_name)
{
  std::string res(str);
  ...
  // TODO: do we need to lock the mutex to read the data??? this would be a contention point
  if (g_data_masking_dict->count(s_dictname_a) == 1 && g_data_masking_dict->count(s_dictname_b) == 1)
  {
    std::vector<std::string> *a = &(g_data_masking_dict->at(s_dictname_a));
    std::vector<std::string> *b = &(g_data_masking_dict->at(s_dictname_b));
    std::vector<std::string>::iterator it = std::find(a->begin(), a->end(), res);
    if (it != a->end())
    {
...
static char * gen_blacklist(UDF_INIT *, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *)
{
...
{
  return {"gen_blacklist",
          Item_result::STRING_RESULT,
          reinterpret_cast<Udf_func_any>(gen_blacklist),
          gen_blacklist_init,
