...
static bool gen_dictionary_load_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  DBUG_ENTER("gen_dictionary_load_init");

  if (args->arg_count != 2)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument list: gen_dictionary_load(dictionary_path, dictionary name)");
    DBUG_RETURN(true);
  }

  if (args->arg_type[0] != STRING_RESULT || args->arg_type[1] != STRING_RESULT)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument type: gen_dictionary_load(string, string)");
    DBUG_RETURN(true);
  }
...
static void gen_dictionary_load_deinit(UDF_INIT *initid)
{
...
*            Once loaded into the registry, a dictionary is used as is, even if the underlying dictionary file changes.
*            To reload a dictionary, first drop it with gen_dictionary_load(), then load it again with gen_dictionary_load().
*
* @param dictionary_path: A string that specifies the path name of the dictionary file.
...
*/
static std::string _gen_dictionary_load(const char *dictionary_path, const char *dictionary_name)
{
...
static char * gen_dictionary_load(UDF_INIT *, UDF_ARGS *args, char *result, unsigned long *length, char *, char *)
{
...
{
  return {"gen_dictionary_load",
          Item_result::STRING_RESULT,
          reinterpret_cast<Udf_func_any>(gen_dictionary_load),
          gen_dictionary_load_init,
          gen_dictionary_load_deinit};
}
