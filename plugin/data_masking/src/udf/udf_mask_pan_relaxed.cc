...
static bool mask_pan_relaxed_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  DBUG_ENTER("mask_pan_relaxed_init");

  if (args->arg_count != 1)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument list: mask_pan_relaxed(string)");
    DBUG_RETURN(true);
  }

  if (args->arg_type[0] != STRING_RESULT)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument type: mask_pan_relaxed(string)");
    DBUG_RETURN(true);
  }
 ...
 *
 * @return The masked payment number as a string. If the argument is shorter than required, it is returned unchanged.
 */
static const char * mask_pan_relaxed(UDF_INIT *initid, UDF_ARGS *args, char *result MY_ATTRIBUTE((unused)), unsigned long *length, char *is_null, char *)
{
  ...
  else
  {
    ...
    if (args->lengths[0] == PAN_LENGTH_1 || args->lengths[0] == PAN_LENGTH_2)
      s = mysql::plugins::mask_inner(args->args[0], args->lengths[0], unmasked_chars_start, unmasked_chars_end, masking_char);
    *length = s.length();
