...
static bool mask_pan_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  ...
  if (args->arg_count != 1)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument list: mask_pan(string)");
    DBUG_RETURN(true);
  }

  if (args->arg_type[0] != STRING_RESULT)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument type: mask_pan(string)");
    DBUG_RETURN(true);
  }
 ...
 */
static const char * mask_pan(UDF_INIT *initid, UDF_ARGS *args, char *result MY_ATTRIBUTE((unused)), unsigned long *length, char *is_null, char *)
{
  DBUG_ENTER("mask_pan");
...
udf_descriptor udf_mask_pan()
{
  return {"mask_pan",
          ...
          reinterpret_cast<Udf_func_any>(mask_pan),
          mask_pan_init,
          mask_pan_deinit};
}
