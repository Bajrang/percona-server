...
static bool gen_rnd_ssn_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  ...
  if (args->arg_count != 0)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument list: gen_rnd_ssn()");
    DBUG_RETURN(true);
  }
...
/**
 * Returns a random U.S. Social Security number with the first and second parts each chosen from a range not used for legitimate numbers
 *
 * @return A random U.S. Social Security number.
 */
static char * gen_rnd_ssn(UDF_INIT *initid, UDF_ARGS *args MY_ATTRIBUTE((unused)), char *result MY_ATTRIBUTE((unused)), unsigned long *length, char *, char *)
{
...
udf_descriptor udf_gen_rnd_ssn()
{
  return {"gen_rnd_ssn",
          ...
          reinterpret_cast<Udf_func_any>(gen_rnd_ssn),
          gen_rnd_ssn_init,
          gen_rnd_ssn_deinit};
}
