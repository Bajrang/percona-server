...
static bool gen_range_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
  ...
  if (args->arg_count != 2)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument list: gen_range(lower, upper)");
    DBUG_RETURN(true);
  }

  if (args->arg_type[0] != INT_RESULT || args->arg_type[1] != INT_RESULT)
  {
    std::snprintf(message, MYSQL_ERRMSG_SIZE, "Wrong argument type: gen_range(long, long)");
    DBUG_RETURN(true);
  }
 ...
 * @param upper: An integer that specifies the upper boundary of the range, which must not be less than the lower boundary.
 *
 * @return A random integer in the range from lower to upper, inclusive, or NULL if the upper argument is less than lower.
 */
static long long gen_range(UDF_INIT *initid MY_ATTRIBUTE((unused)), UDF_ARGS *args, char *is_null, char *is_error)
{
...
udf_descriptor udf_gen_range()
{
  return {"gen_range",
          ...
          reinterpret_cast<Udf_func_any>(gen_range),
          gen_range_init,
          gen_range_deinit};
}
